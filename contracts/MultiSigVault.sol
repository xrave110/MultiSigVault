...
contract MultiSigVault is Ownable, AccessControl {
        ...
        onlyRole(getRoleAdmin(role))
    {
        ...
        require(
            account == msg.sender,
            "You cannot revoke role from your own account"
        );
        approvalsLimit -= 1;
    ...
    function deposit() public payable returns (uint256) {
        //paybale allows to use msg.value
        emit depositDone(msg.value, msg.sender);
        return address(this).balance; // contract reflects the address
    }

    function removeTransaction(uint256 txId) private {
        for (uint256 i = txId; i < transactionLog.length - 1; i++) {
        ...
        returns (uint256)
    {
        ...
        require(
            address(this).balance >= transactionLog[txId]._amount,
            "You do not have enough funds"
        );
        require(
            approvalsLimit <= transactionLog[txId]._signatures.length,
     ...
     */
    function withdrawWithTimeConstraints(uint256 amount)
        public
        payable
        onlyRole(SIGNER)
        returns (uint256)
    {
        ...
        require(
            block.timestamp > startTime + 20 seconds,
            "Not enough time passed since last withdrawal"
        );
    ...
    {
        transactionLog.push(
            Transfer(_to, _amount, transactionLog.length, signature)
        );
    }

    function approve(uint256 txId) public onlyRole(SIGNER) {
        bool approveAllowed = true;
        for (uint256 j; j < transactionLog[txId]._signatures.length; j++) {
